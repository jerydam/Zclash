  ...
  const [rewardAmountRaw, setRewardAmountRaw] = useState<string>(""); // ← exact raw value from contract
  ...
  const truncate4 = (value: string): string => {
    const [whole, dec = ""] = value.split(".");
    return dec ? `${whole}.${dec.slice(0, 4)}` : whole;
  };
  ...
  useEffect(() => {
    ...
    const checkContract = async (
      ...
      try {
        ...
        // ── Store raw value for exact claim, truncated string for display ──
        const rawStr = rewardRaw?.toString() ?? "";
        const fmt = hasReward
          ? truncate4(formatUnits(rewardRaw, tokenDecimals)) + " " + tokenSymbol
          : "";

        if (hasReward && rawStr) {
          setRewardAmountRaw(rawStr); // exact wei/token-unit value, never modified
        }

        console.log("💰 [ClaimCheck] Display reward:", fmt || "(none)");
        console.log("💰 [ClaimCheck] Raw reward:", rawStr || "(none)");
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/quiz/${code}/claim`, {
        ...
        body: JSON.stringify({
          ...
          rewardAmountRaw: rewardAmountRaw, // ← exact raw amount from contract, no rounding
        }),
      });
